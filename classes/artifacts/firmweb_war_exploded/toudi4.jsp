    ...
    <div id="container">
        ...
        <div class="content">
            <dl class="company_center_content">
                ...
                <h1>
                    <em></em>
                <s:message code="toudi.message14"/> </h1>
                </dt>
                ...
                <dd>
                    <form id="searchForm">
                        ...
                        <ul class="reset my_jobs">
                            <li data-id="149594">
                                <h3>
                                    <a   href="">${deliver.name}/${deliver.post}</a>
                                </h3>
                                <div>
                                        ${deliver.sex}/ ${deliver.major}/ ${deliver.education} /
                                            <c:if test="${deliver.audit==1}">审核</c:if>
                                            <c:if test="${deliver.audit==2}">初试</c:if>
                                            <c:if test="${deliver.audit==3}">面试</c:if>
                                            <c:if test="${deliver.audit==4}">通过</c:if>
                                            <c:if test="${deliver.audit==9}">终止流程</c:if>
                                </div>
                                <div class="links">
                                    <a  class="job_edit" href="<%=Const.ROOT%>download?filename=${deliver.resume}"><s:message code="toudi.message2"/></a>
                                    <a  class="job_edit" href="<%=Const.ROOT%>deliver/doupdate4?id=${deliver.id}">通过审核</a>
                                </div>
                            </li>
                        </ul>
                        </c:forEach>
